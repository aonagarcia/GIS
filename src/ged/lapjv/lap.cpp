...

{
  ...
  row  i, imin, numfree = 0, prvnumfree, f, i0, k, freerow, *pred, *freex;
  col  j, j1, j2, endofpath, last, low, up, *collist, *matches;
  cost min, h, umin, usubmin, v2, *d;

  freex = new row[dim];       // list of unassigned rows.
 ...
 //printf("stage 3\n");
  ...
  do
  {
    ...
    while (k < prvnumfree)
    {
      ...
      //std::cout<<"Primera ocasion k:" << k << " umin:" << umin << " susbmin:" << usubmin << " dim:" << dim << " prvnumfree:" << prvnumfree << " i:"<<i << std::endl;

      for (j = 1; j < dim; j++) 
      {
        h = assigncost[i][j] - v[j];
        //std::cout <<"\t" << i <<","<< j << " " << assigncost[i][j] << " " << v[j] << " " << h << std::endl;
        if (h < usubmin)
          if (h >= umin) 
          { 
            usubmin = h; 
      ...
      //std::cout<<"Segunnda ocasion k:" << k << " umin:" << umin << " susbmin:" << usubmin << " dim:" << dim << " prvnumfree:" << prvnumfree << std::endl;
      i0 = colsol[j1];
      if (umin < usubmin) 
      ...
      //std::cout<<"Tercera ocasion k:" << k << " umin:" << umin << " usubmin:" << usubmin << " dim:" << dim << " comp:" << (umin - usubmin) << " prvnumfree:" <<prvnumfree << " i0:"<< i0 << std::endl;
      // (re-)assign i to j1, possibly de-assigning an i0.
      rowsol[i] = j1; 
      colsol[j1] = i;

      if (i0 >= 0)           // minimum column j1 assigned earlier.
        if ((umin - usubmin)>1e-6)
          ...
          freex[numfree++] = i0;
      //std::cout<<"Cuarta ocasion k:" << k << " umin:" << umin << " susbmin:" << usubmin << " dim:" << dim << " prvnumfree:" << prvnumfree << std::endl;
    }
    //std::cout<<"Se repite..."<<std::endl;
  }
  ...
  for (f = 0; f < numfree; f++) 
  {
    freerow = freex[f];       // start row of augmenting path.
